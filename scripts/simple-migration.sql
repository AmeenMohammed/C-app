-- Simple Migration Script for Categories and Items Table Fix
...
-- Step 8: Drop the items_with_seller view (we don't need it anymore)
DROP VIEW IF EXISTS public.items_with_seller;

-- Step 9: Drop the old category column
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- Step 10: Update the get_items_within_range function to use proper join
...
-- Step 12: Test the updated function
...
-- Success message
SELECT '✅ Simple migration completed successfully! Categories are properly linked and unnecessary view removed.' as status;