...
const getPaymobConfig = () => {
  // Try to get from environment variables first (development)
  let apiKey = import.meta.env.VITE_PAYMOB_API_KEY;
  let integrationId = import.meta.env.VITE_PAYMOB_INTEGRATION_ID;
  let iframeId = import.meta.env.VITE_PAYMOB_IFRAME_ID;
  let hmacSecret = import.meta.env.VITE_PAYMOB_HMAC_SECRET;

  // Production fallback - replace these with your actual Paymob values
  if (import.meta.env.PROD && !apiKey) {
    console.log('🌐 Using production Paymob configuration');
